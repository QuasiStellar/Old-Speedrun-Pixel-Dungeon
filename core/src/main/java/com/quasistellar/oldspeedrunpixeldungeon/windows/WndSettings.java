...
import com.quasistellar.oldspeedrunpixeldungeon.ui.CheckBox;
import com.quasistellar.oldspeedrunpixeldungeon.ui.OptionSlider;
		...
		public DisplayTab() {
			...
			OptionSlider scale = new OptionSlider(Messages.get(this, "scale"),
				...
				protected void onChange() {
					if (getSelectedValue() != OldSpeedrunPixelDungeon.scale()) {
						OldSpeedrunPixelDungeon.scale(getSelectedValue());
						OldSpeedrunPixelDungeon.switchNoFade((Class<? extends PixelScene>) OldSpeedrunPixelDungeon.scene().getClass(), new Game.SceneChangeCallback() {
							@Override
			...
			CheckBox chkSaver = new CheckBox( Messages.get(this, "saver") ) {
				...
				protected void onClick() {
					...
					if (checked()) {
						checked(!checked());
						OldSpeedrunPixelDungeon.scene().add(new WndOptions(
							...
							protected void onSelect(int index) {
								if (index == 0) {
									checked(!checked());
									OldSpeedrunPixelDungeon.powerSaver(checked());
								}
							}
						});
					} else {
			...
			if (PixelScene.maxScreenZoom >= 2) {
				...
				chkSaver.checked(OldSpeedrunPixelDungeon.powerSaver());
				add(chkSaver);
			}

			RedButton btnOrientation = new RedButton( OldSpeedrunPixelDungeon.landscape() ?
					Messages.get(this, "portrait")
					: Messages.get(this, "landscape") ) {
				@Override
				protected void onClick() {
					OldSpeedrunPixelDungeon.landscape(!OldSpeedrunPixelDungeon.landscape());
				}
			};
			...
			OptionSlider brightness = new OptionSlider(Messages.get(this, "brightness"),
				...
				@Override
				protected void onChange() {
					OldSpeedrunPixelDungeon.brightness(getSelectedValue());
				}
			};
			brightness.setSelectedValue(OldSpeedrunPixelDungeon.brightness());
			...
			OptionSlider tileGrid = new OptionSlider(Messages.get(this, "visual_grid"),
				...
				@Override
				protected void onChange() {
					OldSpeedrunPixelDungeon.visualGrid(getSelectedValue());
				}
			};
			tileGrid.setSelectedValue(OldSpeedrunPixelDungeon.visualGrid());
	...
	private class UITab extends Group {
		...
		public UITab(){
			...
			RedButton btnSplit = new RedButton(Messages.get(this, "split")){
				@Override
				protected void onClick() {
					OldSpeedrunPixelDungeon.toolbarMode(Toolbar.Mode.SPLIT.name());
			...
			RedButton btnGrouped = new RedButton(Messages.get(this, "group")){
				@Override
				protected void onClick() {
					OldSpeedrunPixelDungeon.toolbarMode(Toolbar.Mode.GROUP.name());
					Toolbar.updateLayout();
				}
			};
			...
			RedButton btnCentered = new RedButton(Messages.get(this, "center")){
				@Override
				protected void onClick() {
					OldSpeedrunPixelDungeon.toolbarMode(Toolbar.Mode.CENTER.name());
					Toolbar.updateLayout();
				}
			};
			...
			CheckBox chkFlipToolbar = new CheckBox(Messages.get(this, "flip_toolbar")){
				...
				protected void onClick() {
					super.onClick();
					OldSpeedrunPixelDungeon.flipToolbar(checked());
					Toolbar.updateLayout();
				}
			};
			chkFlipToolbar.setRect(0, btnGrouped.bottom() + GAP_TINY, WIDTH, BTN_HEIGHT);
			chkFlipToolbar.checked(OldSpeedrunPixelDungeon.flipToolbar());
			add(chkFlipToolbar);
			...
			final CheckBox chkFlipTags = new CheckBox(Messages.get(this, "flip_indicators")){
				...
				protected void onClick() {
					super.onClick();
					OldSpeedrunPixelDungeon.flipTags(checked());
					GameScene.layoutTags();
				}
			};
			chkFlipTags.setRect(0, chkFlipToolbar.bottom() + GAP_TINY, WIDTH, BTN_HEIGHT);
			chkFlipTags.checked(OldSpeedrunPixelDungeon.flipTags());
			add(chkFlipTags);

			OptionSlider slots = new OptionSlider(Messages.get(this, "quickslots"), "0", "4", 0, 4) {
				@Override
				protected void onChange() {
					OldSpeedrunPixelDungeon.quickSlots(getSelectedValue());
					Toolbar.updateLayout();
				}
			};
			slots.setSelectedValue(OldSpeedrunPixelDungeon.quickSlots());
			...
			CheckBox chkImmersive = new CheckBox( Messages.get(this, "nav_bar") ) {
				...
				protected void onClick() {
					super.onClick();
					OldSpeedrunPixelDungeon.immerse(checked());
				}
			};
			chkImmersive.setRect( 0, slots.bottom() + GAP_SML, WIDTH, BTN_HEIGHT );
			chkImmersive.checked(OldSpeedrunPixelDungeon.immersed());
			...
			CheckBox chkFont = new CheckBox(Messages.get(this, "system_font")){
				...
				protected void onClick() {
					...
					OldSpeedrunPixelDungeon.switchNoFade((Class<? extends PixelScene>) OldSpeedrunPixelDungeon.scene().getClass(), new Game.SceneChangeCallback() {
						@Override
						public void beforeCreate() {
							OldSpeedrunPixelDungeon.classicFont(!checked());
						}
			...
			chkFont.checked(!OldSpeedrunPixelDungeon.classicFont());
			add(chkFont);
		}
		...
		public AudioTab() {
			OptionSlider musicVol = new OptionSlider(Messages.get(this, "music_vol"), "0", "10", 0, 10) {
				...
				protected void onChange() {
					...
					OldSpeedrunPixelDungeon.musicVol(getSelectedValue());
				}
			};
			musicVol.setSelectedValue(OldSpeedrunPixelDungeon.musicVol());
			...
			CheckBox musicMute = new CheckBox(Messages.get(this, "music_mute")){
				...
				protected void onClick() {
					super.onClick();
					OldSpeedrunPixelDungeon.music(!checked());
				}
			};
			musicMute.setRect(0, musicVol.bottom() + GAP_TINY, WIDTH, BTN_HEIGHT);
			musicMute.checked(!OldSpeedrunPixelDungeon.music());
			add(musicMute);
			...
			OptionSlider SFXVol = new OptionSlider(Messages.get(this, "sfx_vol"), "0", "10", 0, 10) {
				...
				protected void onChange() {
					...
					OldSpeedrunPixelDungeon.SFXVol(getSelectedValue());
				}
			};
			SFXVol.setSelectedValue(OldSpeedrunPixelDungeon.SFXVol());
			...
			CheckBox btnSound = new CheckBox( Messages.get(this, "sfx_mute") ) {
				...
				protected void onClick() {
					super.onClick();
					OldSpeedrunPixelDungeon.soundFx(!checked());
					Sample.INSTANCE.play( Assets.SND_CLICK );
				}
			};
			btnSound.setRect(0, SFXVol.bottom() + GAP_TINY, WIDTH, BTN_HEIGHT);
			btnSound.checked(!OldSpeedrunPixelDungeon.soundFx());
			add( btnSound );
