...
import com.quasistellar.oldspeedrunpixeldungeon.scenes.TitleScene;
...
public class WndLangs extends Window {
	...
	public WndLangs(){
		...
		for (int i = 0; i < langs.size(); i++){
			...
			RedButton btn = new RedButton(Messages.titleCase(langs.get(i).nativeName())){
				...
				protected void onClick() {
					...
					OldSpeedrunPixelDungeon.switchNoFade(TitleScene.class, new Game.SceneChangeCallback() {
						@Override
						public void beforeCreate() {
							OldSpeedrunPixelDungeon.language(langs.get(langIndex));
			...
			if (OldSpeedrunPixelDungeon.landscape() && i % 2 == 1){
		...
		resize(OldSpeedrunPixelDungeon.landscape() ? WIDTH_L : WIDTH_P, y);
