...
public class WndTextInput extends Window {
	...
	public WndTextInput(final String title, final String initialValue, final int maxLength,
		...
		final int width;
		if (OldSpeedrunPixelDungeon.landscape() && multiLine){
			width = W_LAND_MULTI; //more editing space for landscape users
		} else {
			width = WIDTH;
		}

		OldSpeedrunPixelDungeon.instance.runOnUiThread(new Runnable() {
			@Override
			public void run() {
				...
				textInput = new EditText(OldSpeedrunPixelDungeon.instance);
				...
				OldSpeedrunPixelDungeon.instance.addContentView(textInput, layout);
			}
		});
	}
		...
		if (textInput != null){
			OldSpeedrunPixelDungeon.instance.runOnUiThread(new Runnable() {
				...
				public void run() {
					...
					InputMethodManager imm = (InputMethodManager) OldSpeedrunPixelDungeon
					...
					OldSpeedrunPixelDungeon.updateSystemUI();
