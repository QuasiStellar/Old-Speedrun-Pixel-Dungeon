...
import com.quasistellar.oldspeedrunpixeldungeon.effects.BannerSprites.Type;
...
import com.quasistellar.oldspeedrunpixeldungeon.ui.Archs;
import com.quasistellar.oldspeedrunpixeldungeon.ui.ExitButton;
import com.quasistellar.oldspeedrunpixeldungeon.ui.Icons;
...
import com.quasistellar.oldspeedrunpixeldungeon.windows.WndChallenges;
import com.quasistellar.oldspeedrunpixeldungeon.windows.WndClass;
...
public class StartScene extends PixelScene {
	...
	public void create() {
		...
		updateClass( HeroClass.values()[OldSpeedrunPixelDungeon.lastClass()] );
		...
		Badges.loadingListener = new Callback() {
			...
			public void call() {
				if (Game.scene() == StartScene.this) {
					OldSpeedrunPixelDungeon.switchNoFade( StartScene.class );
				}
			}
		};
	}
	...
	private void startNewGame() {
		...
		if (OldSpeedrunPixelDungeon.intro()) {
			OldSpeedrunPixelDungeon.intro( false );
	...
	private class ChallengeButton extends Button {
		...
		protected void createChildren() {
			...
			image = Icons.get( OldSpeedrunPixelDungeon.challenges() > 0 ? Icons.CHALLENGE_ON :Icons.CHALLENGE_OFF );
			add( image );
		}
		...
		protected void onClick() {
			if (Badges.isUnlocked( Badges.Badge.VICTORY )) {
				StartScene.this.add(new WndChallenges(OldSpeedrunPixelDungeon.challenges(), true) {
					public void onBackPressed() {
						super.onBackPressed();
						image.copy( Icons.get( OldSpeedrunPixelDungeon.challenges() > 0 ?
